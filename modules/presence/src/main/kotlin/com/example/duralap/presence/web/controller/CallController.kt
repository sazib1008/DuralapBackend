...
class CallController(
    ...
    fun getIceServers(): ResponseEntity<CallIceServersResponse> {
        val iceServers = callService.getIceServers()
    ...
    fun rejectCall(
        ...
        @RequestParam(defaultValue = "DECLINED") reason: String
    ): ResponseEntity<CallResponse> {
        val call = callService.rejectCall(callId, userId, reason)
        return ResponseEntity.ok(call)
    }

    @PostMapping("/{callId}/cancel")
    fun cancelCall(@PathVariable callId: String, @RequestParam userId: String): ResponseEntity<CallResponse> {
        val call = callService.cancelCall(callId, userId)
        return ResponseEntity.ok(call)
    }
