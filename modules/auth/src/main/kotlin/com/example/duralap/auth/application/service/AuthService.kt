...
class AuthService(
    ...
    fun register(request: UserCreateRequest): UserResponse {
        ...
        if (userRepository.existsByUsername(secureRequest.username.lowercase())) {
            throw IllegalArgumentException("Username already exists")
        }
        if (userRepository.existsByEmail(secureRequest.email.lowercase())) {
    ...
    fun login(request: LoginRequest): AuthResponse {
        val user = userRepository.findByUsernameOrEmail(request.usernameOrEmail.lowercase())
        ...
        val accessToken = jwtTokenProvider.generateAccessToken(user.username, roles, user.id)
        ...
        val userConversationsDto = UserConversationsDto(userId, emptySet())
    ...
    fun refreshToken(request: TokenRefreshRequest): AuthResponse {
        ...
        val roles = user.roles.map { "ROLE_${it.name}" }.toSet()
        val newAccessToken = jwtTokenProvider.generateAccessToken(user.username, roles, user.id)
