package com.example.duralap.search.service
...
class SearchService(
    ...
    fun searchConversations(userId: String, q: String, cursor: String?, limit: Int): Pair<List<Conversation>, String?> {
        ...
        query.with(Sort.by(Sort.Direction.ASC, "id"))
        query.limit(limit * 5) // Fetch more to account for filtering
        ...
        for (conv in conversations) {
            ...
            if (otherParticipantIds.isEmpty()) {
                // If it's a self-conversation, check if current user matches query
