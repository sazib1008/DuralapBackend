package com.example.duralap.chat.service
...
class ConversationService(
    ...
    fun createConversation(request: ConversationCreateRequest): ConversationResponse {
        ...
        // Publish event to notify participants (safe against Kafka downtime)
        try {
            ...
            kafkaTemplate.send(CONVERSATION_EVENTS_TOPIC, event.id, event)
        } catch (e: Exception) {
            org.slf4j.LoggerFactory.getLogger(ConversationService::class.java).warn("Kafka notification skipped: ${e.message}")
        }
